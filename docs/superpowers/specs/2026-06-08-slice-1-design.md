...
**Phase D complete**: branch slice-1/phase-d — ps sync-upgrades populates the upgrades table from mainnet LCD (golden test 14); DB-driven router dispatches decoders by height with NewStaticRouter for unit tests (test 15); ps fileplugin --bootstrap republishes raw block-meta bytes to pokt.block.{H}; ps consumer block decodes consumer-side via the router and writes the block table; 5 real fixtures (v0.1.0/10/20/28/29) drive block rows + AND-seal (test 16) and self-heal gap recovery (test 17). 5 delegating decoder adapters (no gen/, block header version-invariant). make ci + integration-build lint clean; tests 1-17 green. 2026-06-09.

**Phase E complete**: branch slice-1/phase-e — ps consumer supplier indexes the full supplier lifecycle (MsgStakeSupplier, MsgUnstakeSupplier, 5 event types, Supplier/ServiceConfigUpdate KV) across 5 versions with per-height AND-seal; tests 18-21 green. Key findings and deliverables: (1) dehydrated-Supplier/SCU finding: from v0.1.8 the chain stores Supplier DEHYDRATED in KV — ServiceConfigUpdate records are first-class chain state, not embedded; new table supplier_service_config_update_history (migration 0040) captures them; hydration happens at query time via LEAD() (append-only invariant honored). (2) ADR-022 fan-out: sidecar Bootstrap structurally splits block-{H}-meta + block-{H}-data into per-tx (pokt.tx.{H}.{i}), per-event (pokt.events.{type}.{H}), per-KV (pokt.kv.{store}.{H}) using ONLY version-invariant containers (no poktroll decode in sidecar); BlockEnvelope (pocketscribe.v1 gogo proto) published LAST as the per-height completeness fence (ordering contract). (3) BatchRuntime (ADR-024): buffers fan-out messages per height, flushes in one Postgres tx on envelope arrival; quiet heights flush empty and advance cursor (no freeze); ack-after-commit preserved; buffer dedups by Nats-Msg-Id. (4) Shape-complete lenient registry: 3 gen trees at shape-range starts (v0_1_0 [0..7], v0_1_8 [8..26], v0_1_27 [27..33]) plus stripregister post-gen transform eliminating gogoproto duplicate-enum panic; CI shape-guard (TestSupplierShapeGuard) makes registry completeness mechanical — it BFS-expands the supplier closure across all .shapes/ snapshots and fails CI if any break version lacks a DefaultRegistry() entry. (5) 4 supplier-activity fixture heights extracted from mainnet tarballs (102542/135836/290584/385145) with fully populated expected.json (real pokt1... addresses + amounts); golden blobs for v0_1_8 and v0_1_27 ranges. make ci + integration-build lint clean; tests 1-21 green. 2026-06-10.

**Phase F complete**: branch slice-1/phase-f — version-aware orchestration + multi-version fixture expansion; tests 22-27 green (22 dormant, 23 dynamic required_set, 24 restart-based wakeup, 25 multi-network vs real mainnet/localnet YAML, 26 backfill seal semantics, 27 sidecar 256KiB/1MiB payload caps). Key deliverables: (1) internal/protover — semver-canonical version handling at every boundary (dotted + decoder-dir spellings; v0.1.10 > v0.1.9); (2) spec §4.10 implemented in Go: store.FirstValidHeights/RequiredSet(ctx,H,genesis)/IsSealed(ctx,H,genesis) with DormantHeight sentinel; Phase B non-empty seal guard kept; (3) dormancy gate in Runtime+BatchRuntime (register, log, exit 0; wakeup = restart after ps sync-upgrades); (4) internal/fixturereport — single decode pipeline for expected.json generation AND golden verification (generator==checker), chain-authoritative MainnetUpgrades() (31 applied versions; v0.1.1/v0.1.32 never applied), fixtureextract scan mode + scripts/curate_fixtures.sh; (5) fixture corpus expanded to ALL 24 archivable remaining versions (~80 triplets, 79+ golden-walker subtests; real supplier activity found in v0.1.15 MIGRATION era / v0.1.16/18/19/21/22/24/26/27; v0_1_27 break decoder validated with real chain data); (6) router pinned across all 31 mainnet boundaries (TestDecoderForAllMainnetBoundaries); (7) dormancy/required_set documented in docs/architecture/05-versioning.md; (8) curate-version-fixtures skill covers v0.1.30/v0.1.31/v0.1.33 — PENDING archived data (archeology running on multi-1), tracked in test/fixtures/README.md + golden walker TODO. make ci + integration-build lint clean; tests 1-27 green. 2026-06-10.

**Phase G complete**: branch slice-1/phase-g — hardening + reconciler refresh; **Slice 1 exit criterion (§15) met**. Key deliverables: (1) ADR-024 triggers 2-3 implemented: size valve (`MaxRows`, sync on append) + time valve (`MaxAge`, background sweep) partial-flush through new `store.FlushOnly` (tx WITHOUT cursor advance); handlers accept `env == nil` deriving Position from `Message.TimeUnixNano`; acks + dedup ids retained until the fence (invariant 5 intact). (2) `Pocket-Block-Time` NATS header (ADR-022 amendment) on every fan-out message — sidecar stamps consensus block time so partial flushes honor invariant 1. (3) Orphaned heightBuf eviction with deterministic seen-count protocol: `evicted[height] = len(seen)` at eviction; the fence for an evicted height errors (→ Nak) until the rebuilt buffer reaches the recorded count — a late envelope can never seal a hole regardless of redelivery interleaving; re-eviction keeps max count; restart-safe. (4) Reconciler hardened: immediate first sync, `pocketscribe_reconciler_syncs_total`/`sync_errors_total`, loop unit tests; missing `signal.NotifyContext` found and fixed in `cmd/ps/main.go` (SIGINT/SIGTERM never reached subcommands). (5) Edge-case integration tests: valves + eviction end-to-end, sidecar 256KiB/1MiB caps through the pipeline, migrate-down round-trip (dedicated container), intra-tx fault AFTER partial writes (rollback verified), upgrade boundary v0_1_26→v0_1_27 through the full pipeline with DB-driven router (seal-with-gap decision documented), partial simultaneous restart, restart over an open partially-flushed height. (6) Combined unit+integration coverage tooling (`scripts/covmerge` + `scripts/covgate`, `make coverage`): per-package gate 100% decoders / ≥90% internal/ (composition roots `internal/app/*` excluded — integration-covered wiring; jsonpb marshal test seam closes defensive guards in v0_1_0/v0_1_8/v0_1_27). (7) `make ci` hardened (race detector + integration-tag lint), `ci-full` adds the coverage gate; first GitHub Actions workflow (`.github/workflows/ci.yml`: lint v2.11.4 both tag modes + race + integration/coverage jobs). (8) Per-component READMEs for the 9 major internal packages + doc.go coverage for trivial ones. make ci-full clean (coverage gate exit 0: all packages at bar). 2026-06-11.
